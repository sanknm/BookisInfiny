...
const paths = {
     client: {
          ...
          excludesStyles: [
               `!${clientPath}/components/core-scss/variables.scss`,
               `!${clientPath}/components/core-scss/mixins.scss`
          ],
     ...
     server: {
          ...
          template: [
               `${serverPath}/**/!(*.spec|*.integration).ejs`,
               `${serverPath}/**/!(*.spec|*.integration).xlsx`,
               `${serverPath}/**/!(*.spec|*.integration).css`
          ],
...
gulp.task('inject:scss', () => {
     return gulp.src(paths.client.mainStyle)
          .pipe(plugins.inject(
               gulp.src(_.union(paths.client.styles, ['!' + paths.client.mainStyle].concat(paths.client.excludesStyles)), {read: false})
                    .pipe(plugins.sort()),
               {
                    transform: (filepath) => {
...
gulp.task('clean:dist', () => del([`${paths.dist}/!(.git*|.elasticbeanstalk*|.openshift|Procfile)**`], {dot: true}));
